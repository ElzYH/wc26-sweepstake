...
try:
    ...
    stw, bw = S.req("POST", "/api/cancel_bet", {"player": "Erol", "id": bid, "pin": "WRONG"})
    ck("wrong passcode can't void (403)", stw == 403, (stw, bw[:100]))
    sto, bo = S.req("POST", "/api/cancel_bet", {"player": "Louis", "id": bid, "pin": "ABCD"})
    ck("another player can't void it", json.loads(bo).get("ok") is not True, bo[:120])
    st, b = S.req("POST", "/api/cancel_bet", {"player": "Erol", "id": bid, "pin": "ABCD"})
    j = json.loads(b)
    ...
    st2, b2 = S.req("POST", "/api/cancel_bet", {"player": "Erol", "id": bid, "pin": "ABCD"})
