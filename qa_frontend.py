...
ck("the tabs scrollbar rule was removed", ".tabs::-webkit-scrollbar" not in HTML, None)
...
ck("player cards carry the open-bets potential line", "if they all win" in HTML and "losses already priced in" in HTML and "(p.points+p.bet_potential)" in HTML, None)
...
ck("clock isn't clamped down to a lagging feed minute, but a big drift is capped at feed+2 (72:00 drift, feed 50' -> 52:00, never 72:00)", liveClockText("IN_PLAY", 50, 72*60) === "52:00");
...
// ---- accaLiveStatus(): overall winning/level/losing for an accumulator ----
{
  ...
  ck("acca banked leg + not-yet-started leg -> level (headline reflects the win)", accaLiveStatus(W([{matchId:"x",selection:"HOME",result:"won"},{matchId:"zzz",selection:"HOME"}]), fixOf, liveS) === "level");
  ck("acca all legs banked -> win", accaLiveStatus(W([{matchId:"x",selection:"HOME",result:"won"},{matchId:"y",selection:"HOME",result:"won"}]), fixOf, liveS) === "win");
}
