...
- **Handicap bets:** back a team with a goal start or deficit (−1.5, +1.5, −2.5, +2.5 where offered). Settles on the **90'+extra-time score — penalties don't count** (same basis as Over/Under and exact score, *not* the "to advance" market: a 1–1 game decided on pens means −1.5 **loses** and +1.5 **wins** regardless of the shootout). Half-lines only, so a handicap bet can never push. Handicap and exact-score picks can now go in **accumulators** too — any mix of markets, but only **one pick per game** (same-game combos are correlated and stay blocked). Web only for now (not on Discord `/bet`).
